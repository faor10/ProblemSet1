#En este script se presentan la manera en que se obtuvieron los datos 
...
#Scrape the data that is available at the following website https://ignaciomsarmiento.
...
#Data Cleaning **************************************
#NOTA: Parte de este código fue obtenido de la clase de la complementaria 1 del curso
#BIGData

#Según el diccionario convertimos las variables categóricas en tipo Factor
...
##Solo personas mayores a 18 años y ocupadas
db2 <- db[ which(db$age>=18 & db$ocu==1), ]


#Determinamos si nuestras variables escogidas que explican el ingreso tienen NAs y en que porcentaje

summary(db)

cantidad_na <- sapply(db2, function(x) sum(is.na(x)))
...
db_imputado$ingtot[db_imputado$ingtot == 0] <- 15000 
